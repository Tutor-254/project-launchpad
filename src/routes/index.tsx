...
import { ArrowRight, Sparkles, Compass, Users, BookOpen } from "lucide-react";
...
function Index() {
  const { loading: roleLoading, phase, homeHeroCtas, teachCta } = useInstructorOnboarding();
        ...
        <div className="max-w-7xl mx-auto px-6 py-20 md:py-32 grid md:grid-cols-2 gap-12 items-center">
          <div>
            <span className="inline-flex items-center gap-2 text-xs font-medium bg-brand/10 text-brand px-3 py-1.5 rounded-full mb-6">
              ...
              {phase === "instructor" ? "Your instructor studio" : "A curated learning marketplace"}
            </span>
            <h1 className="font-serif text-5xl md:text-6xl lg:text-7xl leading-[1.05] tracking-tight mb-6">
              {phase === "instructor" ? (
                <>
                  Teach what you<br />
                  <span className="text-brand italic">ship in production.</span>
                </>
              ) : (
                <>
                  Master the craft.<br />
            ...
            <p className="text-lg text-muted-foreground max-w-[52ch] leading-relaxed mb-8">
              {phase === "instructor"
                ? "Manage courses, track enrollments, and reach learners who want depth — not shortcuts."
            ...
            <div className="flex flex-wrap gap-3">
              <Link to={homeHeroCtas.primary.to} search={homeHeroCtas.primary.search}>
                <Button size="lg" className="bg-brand text-brand-foreground hover:bg-brand/90 rounded-lg h-12 px-6">
                  {homeHeroCtas.primary.label} <ArrowRight className="ml-2 size-4" />
                </Button>
              </Link>
              {!roleLoading && (
                <Link to={homeHeroCtas.secondary.to} search={homeHeroCtas.secondary.search}>
                  <Button size="lg" variant="outline" className="rounded-lg h-12 px-6">
                    {homeHeroCtas.secondary.label}
                  </Button>
                </Link>
              )}
            </div>
          ...
          <div className="hidden md:grid grid-cols-2 gap-4">
            {(phase === "instructor"
              ? [
                  { icon: BookOpen, label: "Course studio", desc: "Publish sections, lectures, and previews." },
                  { icon: Users, label: "Learner reach", desc: "Students discover you through search and categories." },
                  { icon: Sparkles, label: "Fair revenue", desc: "Keep up to 90% of every course sale." },
                  { icon: ArrowRight, label: "Analytics", desc: "Track enrollments, ratings, and Q&A backlog." },
                ]
              : [
        ...
        {featured && featured.length > 0 ? (
          ...
          <div className="text-center py-16 border-2 border-dashed border-border rounded-2xl">
            ...
            <Link to={teachCta.to} search={teachCta.search}>
              <Button className="bg-brand text-brand-foreground hover:bg-brand/90">{teachCta.label}</Button>
            </Link>
          </div>
        )}
      </section>
